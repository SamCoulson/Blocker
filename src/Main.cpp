...
{	
	...
	CIntroScreen introscreen( graphics );
	
	if( introscreen.init() == false )
	{
		cout << "Failed to initalise CIntroScreen!" << endl;
		return 1;
	}

	while( introscreen.isQuit() != true)
	{
		SDL_PollEvent( &event );
		introscreen.input( &event );

		introscreen.update();
		introscreen.render();
	}
	
	SDL_Delay( 200 ); // put at start of first iteration of main renderloop to allow for apause between pressing play and starting
	
	// **** DEBUG PURPOSES ***
	//LoadTopScoresFromFile();
	
	CGameArea gamearea( graphics );
	

	if( gamearea.init() == false )
	{
		cout << "Failed to initalise CGameScreen!" << endl;
		return 1;
	}

	while( gamearea.isQuit() != true )
	{
		SDL_PollEvent( &event );
		gamearea.input( &event );
...
/* If you uncomment below exit may stop working for main game screen	
	if( quit == false)
	{
		
		 // Prepare top scores
		//Scores = LoadTopScoresFromFile();

		while( scoreEntered != true )
		{
			SDL_PollEvent( &event );
			//OnInput( &event );
